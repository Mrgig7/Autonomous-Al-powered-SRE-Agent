...
class NotificationManager:
    ...
    async def send(
        ...
        if channels:
            ...
            targets = [
                ...
                if name in channels and n.enabled
            ]
        else:
            targets = [
                (name, n) for name, n in self._notifiers.items()
                if n.should_send(payload)
            ]
        ...
        if self.config.enable_fallback:
            ...
            if failed_channels:
                fallback_results = await self._handle_fallbacks(
                    payload, failed_channels, results
                )
    ...
    async def _parallel_dispatch(
        ...
        tasks = [
            self._safe_send(name, notifier, payload)
    ...
    def _check_rate_limit(self) -> bool:
        ...
        self._rate_limit_counts[key] = [
            ts for ts in self._rate_limit_counts[key]
            if ts > window_start
        ]
...
async def send_fix_notification(
    ...
    level = (
        NotificationLevel.INFO if confidence_score >= 0.8
